...
function toDoDelete(e){ // deleteBtn click Event
    const deleteTarget = e.target.parentNode; // 삭제해야 할 노드
    const selectList = document.querySelectorAll('li'); // li태그 전부 선택
    const arrayList = [...selectList]; // selectList 깊은 복사

    // arrayList안에 deleteTarget의 인덱스 반환
    const deleteIndex = arrayList.indexOf(deleteTarget); 
